#!/bin/bash
# Verify HEADING-COMPENSATED stealthy bias-injection reliably lures the TRUE robot
# into the forbidden zone under no_guard (world-frame +X push, orientation-invariant).
# 3 seeds each rate. Target: high incursion rate (fix the ~1-in-4 laser-frame lure).
cd /home/jim/ros2_motion_planning_tutorials
source /opt/ros/jazzy/setup.bash
source install/setup.bash
OUTDIR=experiment_results/gazebo_s1_s6/hc_verify
mkdir -p "$OUTDIR"
for CFG in bias_hc_slow bias_hc_mid; do
  for S in 0 1 2; do
    echo "======== $CFG seed $S ========"
