...
{Author \MakeLowercase{\textit{et al.}}: PETSE: A Probabilistic Execution-Time Safety Envelope with Continuous Runtime Re-Verification for Mobile Robots}
...
Four scenarios (S1--S4), each targeting a distinct control layer (Fig.~\ref{fig:attack_scenarios}), are designed to force the robot into forbidden zone~$F$:

\textbf{S1 (Direct Hazard Goal).} Coordinates inside $F$ are injected as the navigation goal through prompt injection or command contamination, probing whether the safety layer blocks or reprojects overtly dangerous goals.

\textbf{S2 (Implicit Path Inducement).} Both start and goal lie outside $F$, yet the shortest feasible path cuts through it---exposing defenses that only inspect the goal coordinate and not the trajectory geometry.

\textbf{S3 (Velocity Manipulation + Communication Delay).} The approved goal is safe, but the attacker covertly amplifies execution velocity and injects 0--100\,ms of extra latency, so that cumulative displacement overshoots the planned envelope.

\textbf{S4 (Position Spoofing).} The localization estimate $\hat{p}_t$ is offset so that it appears safe while the true position $p_t$ drifts toward $F$, testing robustness under corrupted state feedback.
...
\subsubsection{Adversarial Formalization and Bound Tightness}\label{sec:adversarial}

The additive margin~\eqref{eq:margin} is, in fact, the exact solution to a minimax adversarial problem, which we state formally next.

\begin{definition}[Adversarial Perturbation Set]\label{def:adv}
At each decision time $t$, an adversary selects a perturbation vector $\delta = (\delta_{\mathrm{est}},\, \delta_{\mathrm{track}},\, \delta_{\mathrm{lat}},\, \delta_{\mathrm{brake}})$ from the capability set
\begin{equation}
\mathcal{D}(t) = \Big\{\delta \;\Big|\;
  \|\delta_i\| \le M_i(t),\; i \in \{\mathrm{est}, \mathrm{track}, \mathrm{lat}, \mathrm{brake}\}
\Big\},
\label{eq:advset}
\end{equation}
where each $M_i(t)$ is the corresponding bound from Lemma~\ref{lem:bounds}. The adversary's objective is to maximize the total displacement $\|p_s - \hat{p}_t\|$ for some $s \in [t,\, t{+}\tau{+}t_b]$, subject to $\delta \in \mathcal{D}(t)$.
\end{definition}

Each perturbation source acts on a temporally or spatially distinct segment of the displacement chain $\hat{p}_t \to p_t \to p_{t+\tau} \to p_{t+\tau+t_b}$. Crucially, no distributional relationship---independence, correlation, or joint density---is imposed, because an adversary can deliberately coordinate the four sources to maximize total displacement.

\begin{proposition}[Tightness of the Additive Bound]\label{prop:tight}
Under the no-independence assumption---i.e., when no constraint beyond~\eqref{eq:advset} is imposed on the joint behavior of perturbation sources---the additive margin $M(t) = \sum_i M_i(t)$ satisfies:
\begin{equation}
M(t) = \max_{\delta \in \mathcal{D}(t)} \max_{s \in [t,\, t+\tau+t_b]} \|p_s - \hat{p}_t\|.
\label{eq:tight}
\end{equation}
That is, $M(t)$ equals the worst-case displacement achievable by any adversary operating within $\mathcal{D}(t)$, and no smaller margin can guarantee safety against all strategies in $\mathcal{D}(t)$.
\end{proposition}

\begin{proof}
Upper bound: by the triangle inequality applied to the displacement chain,
$\|p_s - \hat{p}_t\| \le \|\delta_{\mathrm{est}}\| + \|\delta_{\mathrm{track}}\| + \|\delta_{\mathrm{lat}}\| + \|\delta_{\mathrm{brake}}\| \le \sum_i M_i(t) = M(t)$
for all $\delta \in \mathcal{D}(t)$.
Attainability: the maximum is attained when the adversary selects $\delta_i^* = M_i(t) \cdot \hat{u}$ for all $i$, where $\hat{u}$ is a common unit vector directed toward $\mathcal{F}$. Under collinear alignment, $\|p_s - \hat{p}_t\| = \sum_i M_i(t) = M(t)$. Since this strategy lies within $\mathcal{D}(t)$, the bound is tight.
\end{proof}

\begin{remark}[Conservatism under Practical Conditions]\label{rem:conservatism}
In practice, perfect collinear alignment of all four perturbation sources is unlikely. If the sources were mutually independent and isotropically distributed, the expected displacement would scale as $\sqrt{\sum_i M_i^2}$ rather than $\sum_i M_i$. For the baseline parameters used in this work ($M_{\mathrm{est}}{=}0.412$\,m, $M_{\mathrm{track}}{=}0.05$\,m, $M_{\mathrm{lat}}{=}0.05$\,m, $M_{\mathrm{brake}}{=}0.05$\,m), $\sqrt{\sum_i M_i^2} = 0.421$\,m while $\sum_i M_i = 0.562$\,m, yielding a conservatism ratio of $0.562 / 0.421 \approx 1.33$. The additive form thus inflates the margin by approximately 33\% relative to the independence-assuming bound, in exchange for requiring no distributional assumptions---a trade-off appropriate for safety-critical applications operating under adversarial conditions.
\end{remark}

\begin{remark}[Sufficient vs.\ Necessary Condition]\label{rem:suff}
The enforcement condition $\mathrm{dist}(\hat{p}_t, \mathcal{F}) \ge M(t)$ is a \emph{sufficient} condition for safety: it guarantees non-entry, but its violation does not necessarily imply zone entry. A necessary-and-sufficient condition would require exact knowledge of the joint distribution of all perturbation sources and their temporal correlations---information that is generally unavailable at runtime and that an adversary can deliberately invalidate. The sufficient condition is therefore the appropriate design choice for a runtime safety module operating under adversarial conditions, where false negatives (missed violations) carry far greater cost than false positives (conservative blocking).
\end{remark}

\begin{corollary}[Probabilistic Interpretation of $M_{\mathrm{est}}$]
If $\mathbf{e}_t := p_t - \hat{p}_t$ follows a 2D Gaussian with covariance $\Sigma_{xy}(t)$, then $\Pr(\|\mathbf{e}_t\| \le M_{\mathrm{est}}(t)) \ge 1{-}\varepsilon$. The deterministic guarantee of Theorem~1 is independent of this assumption; if adversarial perturbations invalidate the Gaussian model, the $\lambda_{\max}$-based bound still maintains the sufficient condition.
\end{corollary}
