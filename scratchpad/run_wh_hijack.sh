...
OUT=experiment_results/gazebo_s1_s6/wh_hijack; mkdir -p "$OUT"
SEEDS=5; MAX_RETRY=3
is_flaky () { python3 -c "
import json,sys
  ...
  while [ "$valid" -lt "$SEEDS" ]; do
    ...
    while [ "$attempt" -lt "$MAX_RETRY" ]; do
      ...
      python3 -u src/geofence_enforcer/experiments/run_gazebo_s1_s6.py \
        ...
        --intensity wh_hijack --output "$OUT/res_${3}_v${valid}.jsonl" > "$OUT/${3}_v${valid}.log" 2>&1
...
for M in no_guard selp_proper cbf_inflated geofence; do
  run_cell wh_hijack "$M" "wh_hijack_${M}"
done
echo "WH_HIJACK DONE $(date +%H:%M:%S)"
